...
public class BoardServiceImpl implements BoardService {
	...
	public boolean deleteBoardByArticleId(String id) {
		...
		if (!AuthUtils.isAdmin() || !AuthUtils.getUserEmail().equals(board.getEmail())) {
			throw new HelloSpringException("잘못된 접근입니다.", "errors/404");
		}
	...
	public boolean updateBoardByArticleId(UpdateVO updateVO) {
		...
		if (!AuthUtils.isAdmin() || !AuthUtils.getUserEmail().equals(updateVO.getEmail())) {
			throw new HelloSpringException("잘못된 접근입니다.", "errors/404");
		}
	...
	public boolean deleteAllBoard() {
		if (!AuthUtils.isSuperAdmin()) {
			throw new HelloSpringException("권한이 없습니다.", "errors/404");
		}

		List<String> deleteFileGroupIdList = this.filesDao.selectFileGroupIdFromBoard();
		List<String> replyFileGroupIdList = this.filesDao.selectFileGroupIdFromReply();

		List<String> deleteTargetFileGroupIdList = new ArrayList<String>();
		deleteTargetFileGroupIdList.addAll(deleteFileGroupIdList);
		deleteTargetFileGroupIdList.addAll(replyFileGroupIdList);

		deleteTargetFileGroupIdList.forEach(fileGroupId -> {
			// TODO 게시판과 댓글의 파일만 삭제하도록 변경

			List<String> pathList = this.filesDao.selectFilePathByFileGroupId(fileGroupId);
			if (pathList != null) {
			...
			this.filesDao.deleteFileByFileGroupId(fileGroupId);
			this.filesDao.deleteFilesGroupByFileGroupId(fileGroupId);
		});

		this.repliesDao.deleteALLReply();
